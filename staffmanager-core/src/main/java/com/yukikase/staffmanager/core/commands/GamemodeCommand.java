...
@Component
@Permission({IStaffMode.PERMISSION, GamemodeCommand.GAMEMODE_PERMISSION})
public class GamemodeCommand implements ICommand {
    public static final String GAMEMODE_PERMISSION = "gamemode";
    public static final String GAMEMODE_OTHER = "other";
    ...
    @Alias(subcommand = "1")
    @Alias(subcommand = "c")
    public boolean onCreative(CommandSender sender, Command command, String[] args) {
    ...
    @Alias(subcommand = "0")
    @Alias(subcommand = "s")
    public boolean onSurvival(CommandSender sender, Command command, String[] args) {
    ...
    @Alias(subcommand = "4")
    @Alias(subcommand = "sp")
    public boolean onSpectator(CommandSender sender, Command command, String[] args) {
    ...
    @Alias(subcommand = "3")
    @Alias(subcommand = "a")
    public boolean onAdventure(CommandSender sender, Command command, String[] args) {
    ...
    private boolean changeGamemode(CommandSender sender, GameMode gamemode, String[] args) {
        if (sender instanceof Player player) {
            if (!staffMode.isInStaffMode(player) && !this.permissionHandler.playerHasPermission(player, GAMEMODE_PERMISSION))
    ...
    private boolean changeGamemode(CommandSender sender, Player other, GameMode gamemode) {
        if (sender instanceof Player player) {
            if (!staffMode.isInStaffMode(player) && !this.permissionHandler.playerHasPermission(player, GAMEMODE_PERMISSION, GAMEMODE_OTHER))
                throw new UnauthorizedException(PLAYER_NO_PERMISSION_GAMEMODE_OTHER);
            if (!this.permissionHandler.playerHasPermission(player, IStaffMode.PERMISSION, GAMEMODE_PERMISSION, GAMEMODE_OTHER))
